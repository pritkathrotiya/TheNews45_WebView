...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f30mX = new float[numPoints];
            this.f31mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
                this.f30mX[i] = position[0];
                this.f31mY[i] = position[1];
            }
            if (((double) Math.abs(this.f30mX[0])) > 1.0E-5d || ((double) Math.abs(this.f31mY[0])) > 1.0E-5d || ((double) Math.abs(this.f30mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f31mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                StringBuilder sb = new StringBuilder();
                sb.append("The Path must start at (0,0) and end at (1,1) start: ");
                sb.append(this.f30mX[0]);
                String str = ",";
                sb.append(str);
                sb.append(this.f31mY[0]);
                sb.append(" end:");
                sb.append(this.f30mX[numPoints - 1]);
                sb.append(str);
                sb.append(this.f31mY[numPoints - 1]);
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f30mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f30mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
