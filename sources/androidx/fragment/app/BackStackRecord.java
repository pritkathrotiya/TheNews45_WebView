...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, OpGenerator {
    ...
    static final class C0172Op {
        ...
        C0172Op(int cmd2, Fragment fragment2) {
            this.cmd = cmd2;
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                C0172Op op = (C0172Op) this.mOps.get(opNum);
                switch (op.cmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
    ...
    private void doAddOp(int containerViewId, Fragment fragment, String tag, int opcmd) {
        ...
        addOp(new C0172Op(opcmd, fragment));
    }
            ...
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        C0172Op thatOp = (C0172Op) record.mOps.get(thoseOpIndex);
            ...
            if (i != 1) {
                if (i == 2) {
                        ...
                        if (old.mContainerId == containerId) {
                            if (old == f) {
                                ...
                                if (old == oldPrimaryNav) {
                                    this.mOps.add(opNum, new C0172Op(9, old));
                                    opNum++;
                                    oldPrimaryNav = null;
                                }
                                C0172Op removeOp = new C0172Op(3, old);
                    ...
                    if (op.fragment == oldPrimaryNav) {
                        this.mOps.add(opNum, new C0172Op(9, op.fragment));
                        opNum++;
                        oldPrimaryNav = null;
                    }
                } else if (i != 7) {
                    if (i == 8) {
                        this.mOps.add(opNum, new C0172Op(9, oldPrimaryNav));
                        opNum++;
    ...
    public boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (isFragmentPostponed((C0172Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
